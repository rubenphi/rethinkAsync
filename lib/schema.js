...
function validateCustomizedValidator(doc, schema, prefix) {
  ...
  if (typeof schema.validator === 'function' || schema.validator.constructor.name === "AsyncFunction") {
    if (schema.validator(doc) === false) {
      throw new Errors.ValidationErrors.ValidationError("Validator for the field "+prefix+" returned `faise`.");
    }
  }
}
